...
def save_config():
    with open('output/config.txt', 'w') as fout:
...
###################################
print("Loading Data...")
if not os.path.exists('input/samples.npy') or not os.path.exists('input/lengths.npy'):
...
np.save('output/rand.npy', rand_vecs)
...
if PLAY_ONLY:
    ...
    make_rand_songs_normalized('output/', rand_vecs)
    for i in range(20):
        x_test_song = x_train[i:i+1]
        y_song = model.predict(x_test_song, batch_size=BATCH_SIZE)[0]
        midi.samples_to_midi(y_song, 'output/gt' + str(i) + '.mid', 16)
    exit(0)
